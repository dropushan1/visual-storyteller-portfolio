...
const ThumbnailCard = ({ driveId, index }: ThumbnailCardProps) => {
    const [isLoaded, setIsLoaded] = useState(false);
    const [isHovered, setIsHovered] = useState(false);
    const cardRef = useRef<HTMLDivElement>(null);
    const { ref, isVisible } = useScrollAnimation(0.1);

    // Use the Google Drive thumbnail service for reliable direct image loading
    ...
    return (
        <div ref={ref}>
            <div
                ref={cardRef}
                className={`video-container glass-card aspect-video scroll-scale ${staggerClass} ${isVisible ? 'visible' : ''} overflow-hidden`}
            ...
            >
                ...
                <img
                    ...
                    className={`w-full h-full object-cover transition-all duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'} ${isHovered ? 'scale-110' : 'scale-100'}`}
                    onLoad={() => setIsLoaded(true)}
                    onError={() => setIsLoaded(true)} // Set loaded to true even on error to hide shimmer
                />
